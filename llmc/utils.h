...
#define fopenCheck(path, mode) fopen_check(path, mode, __FILE__, __LINE__) // NOLINT
...
#define freadCheck(ptr, size, nmemb, stream) fread_check(ptr, size, nmemb, stream, __FILE__, __LINE__) // NOLINT
...
#define fcloseCheck(fp) fclose_check(fp, __FILE__, __LINE__) // NOLINT
...
#define scloseCheck(sockfd) sclose_check(sockfd, __FILE__, __LINE__) // NOLINT
...
#define fseekCheck(fp, off, whence) fseek_check(fp, off, whence, __FILE__, __LINE__) // NOLINT
...
#define fwriteCheck(ptr, size, nmemb, stream) fwrite_check(ptr, size, nmemb, stream, __FILE__, __LINE__) // NOLINT
...
#define mallocCheck(size) malloc_check(size, __FILE__, __LINE__) // NOLINT
...
#define tokenCheck(tokens, count, vocab) token_check(tokens, count, vocab, __FILE__, __LINE__) // NOLINT
...
extern inline int find_max_step(const char* output_log_dir) {
    ...
    if (output_log_dir == nullptr) { return -1; }
    DIR* dir;
    struct dirent* entry;
    int max_step = -1;
